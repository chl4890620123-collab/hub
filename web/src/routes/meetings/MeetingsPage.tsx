...
function RecordingPanel({ projectId, onJobStarted }: { projectId: number; onJobStarted: (jobId: number) => void }) {
  ...
  return (
    <Card>
      <CardHeader><CardTitle>실시간 녹음</CardTitle></CardHeader>
    ...
    <Card>
      ...
      <CardContent className="flex flex-wrap items-end gap-3">
        ...
        <div>
          ...
          <label htmlFor="meeting-file" className={cn(buttonVariants({ variant: 'outline', size: 'md' }), upload.isPending && 'pointer-events-none opacity-50')}><Upload size={14} /> 선택하면 바로 업로드</label>
          <input id="meeting-file" ref={fileRef} type="file" accept="audio/*" disabled={upload.isPending} className="sr-only" onChange={(e) => { const file = e.target.files?.[0]; if (file) upload.mutate(file); }} />
        </div>
...
export function MeetingsPage() {
  ...
  const { data: recentJobs } = useQuery({
    queryKey: ['recent-jobs', projectId],
  ...
  return (
    <div>
      ...
      {meetingJobs.length > 0 && (
        <Card className="mt-5">
          <CardHeader><CardTitle>최근 회의 처리 내역</CardTitle></CardHeader>
          <CardContent className="flex flex-col gap-2">
            <p className="text-xs text-ink-400">새로고침하거나 다른 화면에 다녀와도 최근 회의 분석을 다시 열 수 있습니다.</p>
            {meetingJobs.map((job) => (
              <button key={job.id} type="button" onClick={() => setActiveJobId(job.id)} className={cn('flex items-center justify-between rounded-md border border-ink-200 px-3 py-2 text-left text-sm hover:bg-ink-50 dark:hover:bg-ink-200', activeJobId === job.id && 'border-accent-400')}>
                <span>회의 분석 #{job.id}</span>
                <span className="text-xs text-ink-400">{job.status === 'SUCCESS' ? '완료' : job.status === 'FAILED' ? '실패' : '처리 중'} · {job.progress}%</span>
              </button>
            ))}
          </CardContent>
        </Card>
