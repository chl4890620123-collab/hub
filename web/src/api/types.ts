...
export interface DecisionRow { id: number; statement: string; confidence: ConfidenceLevel; review_status: ReviewStatus; created_at: string; }
export interface ChangeItemRow { id: number; category: string; before_text: string; after_text: string; reason: string; review_status: ReviewStatus; created_at: string; }
export interface ChangeCompareItem { category: string; before: string; after: string; reason: string; }
export interface ChangeCompareResponse { changes: ChangeCompareItem[]; }
...
export interface ReassignmentRequest { id: number; todo_id?: number; project_id?: number; former_assignee_id?: number | null; former_assignee_name?: string | null; title?: string; task_status?: TaskStatus; due_date?: string | null; reason?: string; created_at?: string; [key: string]: unknown; }
export interface SensitiveTerm { id: number; term: string; createdAt?: string; [key: string]: unknown; }
export interface SearchRule { id: number | null; projectId: number | null; name: string; aliases: string[]; patterns: string[]; targetFile?: string | null; mode: 'SMART' | 'FULL'; priority: number; active: boolean; managed: boolean; }
...
export interface SpreadsheetFileRow { id: number; projectId: number; name: string; ownerId: number; columns: SpreadsheetColumn[]; passwordProtected: boolean; passwordHint: string | null; rowCount: number; createdAt: string; updatedAt: string; }
export interface SpreadsheetDataRow { id: number; fileId: number; position: number; cells: Record<string, string>; updatedAt: string; }
export interface SpreadsheetUnlocked { file: SpreadsheetFileRow; rows: SpreadsheetDataRow[]; }
