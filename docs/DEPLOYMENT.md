...
설치(첫 배포)에서는 아무도 승인할 수 없으므로, `D:\server-data\hub\runtime\.env`에
`HUB_BOOTSTRAP_ADMIN_PASSWORD`(12자 이상, 영문+숫자)를 직접 정해서 넣고 배포/재기동하면
`BootstrapService`가 그 비밀번호로 관리자 계정 하나를 만듭니다 (로그인 아이디는
`HUB_BOOTSTRAP_ADMIN_LOGIN_ID`, 기본값 `admin`). 비밀번호는 이 서버의 `.env`에만 있어야 하며
**절대 git에 커밋하지 않습니다** - git에 값을 적어두는 코드/마이그레이션은 두지 않습니다. 최초
로그인 시 비밀번호 변경이 강제됩니다. 이미 관리자가 있으면 이 값은 무시되고 아무 것도 만들지
않으므로, 첫 관리자 승인 후에는 `HUB_BOOTSTRAP_ADMIN_PASSWORD`를 비워 둬도 됩니다.
