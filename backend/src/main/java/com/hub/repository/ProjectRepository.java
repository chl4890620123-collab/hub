...
public class ProjectRepository {
    ...
    public List<Project> listForUser(long userId, boolean admin) {
        if (admin) {
            return jdbc.query("SELECT p.id,p.name,p.description,p.created_by FROM project p ORDER BY p.id",
                    (rs,n)->new Project(rs.getLong("id"),rs.getString("name"),rs.getString("description"),rs.getLong("created_by"),"ADMIN"));
        }
        return jdbc.query("SELECT p.id,p.name,p.description,p.created_by FROM project p JOIN project_member pm ON pm.project_id=p.id WHERE pm.user_id=? ORDER BY p.id",
                (rs,n)->new Project(rs.getLong("id"),rs.getString("name"),rs.getString("description"),rs.getLong("created_by"),"MEMBER"), userId);
    }
    ...
    public List<java.util.Map<String,Object>> listMembers(long projectId) {
        return jdbc.queryForList("""
                SELECT u.id user_id,u.display_name,u.email,'MEMBER' project_role,u.account_status
