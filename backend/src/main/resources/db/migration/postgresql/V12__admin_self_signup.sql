-- separate ADMIN signup metadata without granting ADMIN before approval.
-- requested_role is the requested account role; global_role remains the effective authorization role.
...
-- One-row lock used to serialize the very first ADMIN claim across concurrent requests.
CREATE TABLE system_guard (
  guard_key VARCHAR(80) PRIMARY KEY,
  guard_value VARCHAR(200) NOT NULL
);
INSERT INTO system_guard(guard_key,guard_value) VALUES('FIRST_ADMIN','OPEN') ON CONFLICT (guard_key) DO NOTHING;
