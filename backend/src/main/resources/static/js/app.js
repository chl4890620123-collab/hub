...
async function init(){
 ...
 if(currentUser.globalRole==='ADMIN'){document.getElementById('adminNav').hidden=false;document.getElementById('newProjectBtn').hidden=false;document.querySelectorAll('[data-view="review"],.connectorForm').forEach(node=>node.hidden=false);const connectorMemberNotice=document.getElementById('connectorMemberNotice');if(connectorMemberNotice)connectorMemberNotice.hidden=true;const memoryForm=document.getElementById('memoryForm');if(memoryForm)memoryForm.hidden=false;const m=document.getElementById('compactAdminBtn');if(m)m.hidden=false;const dashOverview=document.getElementById('dashboardAdminOverview');if(dashOverview)dashOverview.hidden=false;const dashLink=document.getElementById('dashboardAdminLink');if(dashLink)dashLink.hidden=false;}else{document.getElementById('newProjectBtn').hidden=true;document.querySelectorAll('[data-view="review"],[data-compact-view="review"],.connectorForm').forEach(node=>node.hidden=true);const memoryForm=document.getElementById('memoryForm');if(memoryForm)memoryForm.hidden=true;}
 await loadProjects();setProjectAvailability(Boolean(currentProject));document.getElementById('monthInput').value=localMonth();if(currentProject)await refreshAll();if(currentUser.globalRole==='ADMIN')loadAdmin().catch(()=>{});switchView(preferredInitialView());
}
...
function bindForms(){
 ...
 document.getElementById('memoryForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);await api(`/api/projects/${currentProject}/memory`,{method:'POST',body:JSON.stringify({key:f.get('key'),value:f.get('value'),type:'FACT'})});e.target.reset();await loadMemory();flash('프로젝트 요약에 저장했습니다.');};
...
async function refreshAll(){if(!currentProject)return;await loadMembers();await Promise.all([loadTodos(),loadReview(),loadTimeline(),loadMemory(),loadDocuments(),loadProcessingJobs(),loadTopSearches(),loadConnectorStatus()]);renderSummary();renderDashboardTodos();}
...
async function loadTimeline(){const data=await api(`/api/projects/${currentProject}/timeline`);const render=root=>{root.replaceChildren();data.slice(0,100).forEach(x=>{const i=el('div','item');i.appendChild(el('strong','',x.title));i.appendChild(el('div','muted',`${eventTypeLabel(x.eventType)} · ${x.happenedAt}`));if(x.description)i.appendChild(el('p','',x.description));root.appendChild(i);});};render(document.getElementById('timelineList'));render(document.getElementById('dashboardTimeline'));}
async function loadMemory(){const data=await api(`/api/projects/${currentProject}/memory`),root=document.getElementById('memoryList');root.replaceChildren();data.forEach(x=>{const i=el('div','item');i.appendChild(el('strong','',x.memoryKey));i.appendChild(el('p','',x.memoryValue));i.appendChild(el('div','muted',`${x.memoryType==='FACT'?'확정된 사실':'프로젝트 정보'} · ${reviewStatusLabel(x.reviewStatus)}`));root.appendChild(i);});}
...
async function openVersion(versionId){const d=await api(`/api/versions/${versionId}`);openViewer({title:value(d,'original_name'),meta:`${sourceTypeLabel(value(d,'source_type'))} · ${value(d,'version_no')}번째 저장본`,evidence:'문서 전체 보기',summary:value(d,'summary')||'요약 없음',text:value(d,'full_text')||''});}
...
function renderMaterialResults(root,data,query=''){
 ...
 data.forEach((x,index)=>{
   ...
   if(isSafeExternalUrl(x.sourceUrl)){
     ...
     const b=el('button','ghost','Hub 원문 보기');b.onclick=async()=>{const d=await api(`/api/chunks/${x.evidenceId}`);openViewer({title:value(d,'original_name'),meta:`${value(d,'paragraph_ref')||'원문 근거'}`,evidence:value(d,'content'),summary:value(d,'summary')||'요약 없음',text:value(d,'full_text')||'',quote:value(d,'content')});};actions.appendChild(b);
   }
