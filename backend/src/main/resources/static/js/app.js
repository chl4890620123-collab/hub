...
async function refreshAll(){if(!currentProject)return;await loadMembers();const isAdmin=currentUser?.globalRole==='ADMIN';await Promise.all([loadTodos(),...(isAdmin?[loadReview()]:[]),loadTimeline(),loadMemory(),loadDocuments(),loadProcessingJobs(),loadTopSearches(),loadConnectorStatus()]);renderSummary();renderDashboardTodos();}
