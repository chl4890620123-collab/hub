...
CREATE TABLE meeting (
  ...
  stt_status VARCHAR(30) NOT NULL DEFAULT 'PENDING',
...
CREATE TABLE evidence (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  version_id BIGINT REFERENCES document_version(id),
  chunk_id BIGINT REFERENCES document_chunk(id),
  transcript_segment_id BIGINT REFERENCES transcript_segment(id),
...
CREATE TABLE ai_run (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  project_id BIGINT NOT NULL REFERENCES project(id) ON DELETE CASCADE,
  document_version_id BIGINT REFERENCES document_version(id),
...
CREATE TABLE todo (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  project_id BIGINT NOT NULL REFERENCES project(id) ON DELETE CASCADE,
  source_document_version_id BIGINT REFERENCES document_version(id),
  source_meeting_id BIGINT REFERENCES meeting(id),
  title VARCHAR(500) NOT NULL,
  description CLOB,
  assignee_id BIGINT REFERENCES app_user(id),
  assignee_text VARCHAR(200),
  assignee_suggestion_text VARCHAR(200),
  due_date DATE,
...
CREATE TABLE todo_evidence (
  ...
  PRIMARY KEY (todo_id, evidence_id)
);
...
CREATE TABLE decision_evidence (
  decision_id BIGINT NOT NULL REFERENCES decision_candidate(id) ON DELETE CASCADE,
  evidence_id BIGINT NOT NULL REFERENCES evidence(id) ON DELETE CASCADE,
...
CREATE TABLE change_analysis (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  project_id BIGINT NOT NULL REFERENCES project(id) ON DELETE CASCADE,
  before_version_id BIGINT NOT NULL REFERENCES document_version(id),
  after_version_id BIGINT NOT NULL REFERENCES document_version(id),
...
CREATE TABLE change_item (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  analysis_id BIGINT NOT NULL REFERENCES change_analysis(id) ON DELETE CASCADE,
  category VARCHAR(40),
  before_text CLOB,
  after_text CLOB,
  reason CLOB,
  review_status VARCHAR(30) NOT NULL DEFAULT 'AI_GENERATED'
);

CREATE TABLE change_evidence (
  change_item_id BIGINT NOT NULL REFERENCES change_item(id) ON DELETE CASCADE,
  evidence_id BIGINT NOT NULL REFERENCES evidence(id) ON DELETE CASCADE,
  evidence_side VARCHAR(10) NOT NULL CHECK (evidence_side IN ('BEFORE','AFTER')),
  PRIMARY KEY (change_item_id, evidence_id, evidence_side)
);
...
CREATE TABLE external_item (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  project_id BIGINT NOT NULL REFERENCES project(id) ON DELETE CASCADE,
  connector_account_id BIGINT REFERENCES connector_account(id) ON DELETE SET NULL,
  ...
  raw_metadata CLOB,
  imported_document_id BIGINT REFERENCES document(id),
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE (project_id, connector_account_id, external_id)
);
